    ...
    <div class="card-body">
        ...
        <div class="card-text">
            ...
            @php
                ...
                'pageLength' => 200, // Configuración por defecto de la cantidad de entradas por página
            ...
            <x-adminlte-datatable id="table1" :heads="$heads" :config="$config" striped hoverable bordered compressed with-buttons>
                @foreach ($teachers as $teacher)
                    <tr>
                        <td>{{ $key ++ }}</td>
                        <td> {{ $teacher->user->delegations->region->region }} - {{ $teacher->user->delegations->region->sede }} </td>
                        <td> {{ $teacher->user->delegations->delegacion }} </td>
                        <td> {{ $teacher->user->nombre }} {{ $teacher->user->apaterno }} {{ $teacher->user->amaterno }} </td>
                        ...
                        <td> 
                            {!! Form::open(['route' => ['usuario.teacher.constancia',$teacher->codigo_id], 'method' => 'POST', 'style' => 'display: inline', 'target' => '_blank']) !!}
                                @csrf
                                {!! Form::button('<i class="fa fa-xl fa-fw fa-file-pdf"></i>', ['class' => 'btn btn-sm btn-default text-teal mx-1 shadow', 'type' => 'submit']) !!}
                            {!! Form::close() !!}
                            <a href="{{route('usuario.teacher.edit',$teacher->slug)}}" class="btn btn-sm btn-default text-primary mx-1 shadow" >
                                <i class="fa fa-xl fa-fw fa-pen"></i>
                            </a>
                            {!! Form::open(['route' => ['usuario.teacher.destroy',$teacher->slug], 'method' => 'DELETE', 'class' => 'formEliminar', 'title' => 'Eliminar' ,'style' => 'display: inline']) !!}
                                @csrf
